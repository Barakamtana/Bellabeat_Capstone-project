...
# group data by week or by date to get a clear picture of correlation
...
```{r}
CaloriesIntensitiesSteps_Daily_df <- weekDate_grouping(CaloriesIntensitiesSteps_df, "date")
...
```{r}
line_plot(CaloriesIntensitiesSteps_Daily_df, "Calories_Total",
          "TotalIntensity_Total")
```
...
for (i in random_Ids){
  ...
  line_plot(filtered_df,
            "Calories_Total", "TotalIntensity_Total"
  )
}
...
```{r}
# We'll perform weekly Weighted averages since to get a better glimpse of the correlation

CaloriesIntensitiesSteps_Weekly_df <- CaloriesIntensitiesSteps_df %>%
  mutate(week = week(ActivityHour)) %>%
  group_by(week) %>%
  summarize(
    weekly_calories = mean(Calories),
    weekly_Intensity = mean(TotalIntensity),
    weekly_Steps = mean(StepTotal)
    )
CaloriesIntensitiesSteps_Weekly_df
```
...
```{r}
CaloriesIntensitiesSteps_Weekly_df <- Weekly_grouping(CaloriesIntensitiesSteps_df)
CaloriesIntensitiesSteps_Weekly_df
```

```{r}
# Group by athlete and week, then summarize the data
weekly_data <- CaloriesIntensitiesSteps_df %>%
  mutate(Week = format(ActivityHour, "%Y-%U")) %>%  # Create year-week column
  group_by(Id, Week) %>%  # Group by Id and week
  summarize(
    # get weekly cumulative sums 
    Total_Calories = sum(Calories, na.rm = TRUE),
    Total_Intensity = sum(TotalIntensity, na.rm = TRUE),
    Total_Step = sum(StepTotal, na.rm = TRUE),
    # get weekly mean
    Avg_Calories = mean(Calories, na.rm = TRUE),  
    Avg_Intensity = mean(TotalIntensity, na.rm = TRUE),
    Avg_Step = mean(StepTotal, na.rm = TRUE),
    .groups = "drop"
  )
...
```{r}
line_plot(CaloriesIntensitiesSteps_Weekly_df, "weekly_calories",
          "weekly_Intensity")
```

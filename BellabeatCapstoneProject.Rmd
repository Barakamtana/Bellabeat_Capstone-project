...
```{r}
error_df <-  dailyActivity_merged_df[, c('TotalDistance', 'TrackerDistance')]
...
print(paste("Percentage of devices with errors",
            (length(error_df)/length(total_distance))*100))

```
#### with a 12.5 percent of Bellabeat devices not giving accurate data, this is significantly high which show devices should be improves,
...
```
```{r}
line_plot <- function(df, x_col, y_col, i){
    plot <- ggplot (data = df) +
      geom_line(mapping = aes(x= .data[[x_col]] , y= .data[[y_col]] )) +
...
```{r}
# Loop through the list and see if key has minuted in it
minutes_dfs = list()
...
```{r}
# See if the the remainin dfs in minutes_df have similar values in ActivityMinute column since the are similar in shape
...
```{r}
IDS <- c(unique(Minuted_Merged_df[["Id"]]))

random_Ids = IDS[ceiling(runif((ceiling(length(IDS)*0.5)), max = length(IDS)))]

for (i in random_Ids){
  filtered_df <- Minuted_Merged_df %>%
    filter(Id == i)
